...
(function() {
  ...
  Humanize.compactInteger = function(input, decimals) {
    var _length, bigNumPrefixes, decimalIndex, decimalPart, decimalPartArray, j, len1, length, number, numberLength, numberLengths, output, outputNumber, signString, unsignedNumber, unsignedNumberCharacterArray, unsignedNumberString, wholePart, wholePartArray;
    if (decimals == null) {
      decimals = 0;
    }
    ...
    for (j = 0, len1 = numberLengths.length; j < len1; j++) {
      _length = numberLengths[j];
  ...
  Humanize.times = function(value, overrides) {
    ...
    if (isFinite(value) && value >= 0) {
      ...
      if (overrides[number] != null) {
        ...
        return "" + (((ref = smallTimes[number]) != null ? ref.toString() : void 0) || number.toString() + ' times');
      }
    }
  };
  ...
  Humanize.pace = function(value, intervalMs, unit) {
    var f, j, len1, prefix, rate, relativePace, roundedPace, timeUnit;
    if (unit == null) {
      unit = 'time';
    }
    ...
    for (j = 0, len1 = timeFormats.length; j < len1; j++) {
      f = timeFormats[j];
  ...
  Humanize.titlecase = Humanize.titleCase = function(string) {
    ...
    splitOnHyphensRegex = /-/;
    doTitleCase = (function(_this) {
      return function(_string, hyphenated, firstOrLast) {
        var index, j, len1, stringArray, titleCasedArray, word;
        if (hyphenated == null) {
          hyphenated = false;
        }
        if (firstOrLast == null) {
        ...
        for (index = j = 0, len1 = stringArray.length; j < len1; index = ++j) {
